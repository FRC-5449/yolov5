...
class MyGUI(QMainWindow):
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture(1)  # webcam object
        self.pixmap = None
        ...
        # points in 3D
        self.capturedObjectPointsLR = [[i * x, j * y, 0] for i in range(self.points_in_row, 0, -1) for j in
                                       range(self.points_in_column, 0, -1)]
    ...
    def doneClicked(self):
        if self.confirmedImagesCounter < 3:
            ...
            QMessageBox.question(self, 'Warning!',
                                 "the number of captured photos should be at least 3. Please take " + str(
                                     rem) + " more photos", QMessageBox.Ok)
    ...
    def built_in_calib(self):  # not used!
        ...
        imgpoints = np.array([np.squeeze(p, axis=0) for k in sorted(self.capturedImagePoints.keys()) for p in
                              self.capturedImagePoints[k]])
    ...
    def loss_function(self, x):  # not used!
        ...
        objpoints[:, 2] = 1.0
        imgpoints = np.array(
            [np.squeeze(p) for k in sorted(self.capturedImagePoints.keys()) for p in self.capturedImagePoints[k]])
        imgpoints = np.append(imgpoints, np.ones((imgpoints.shape[0], 1)), axis=1)  # append 1 to each 2D point
    ...
    def MLE(self, A, Rt):  # not used!
        x = [A[0, 0], A[0, 1], A[0, 2], A[1, 1], A[1, 2], Rt[0, 0], Rt[0, 1], Rt[1, 0], Rt[1, 1], Rt[2, 0], Rt[2, 1],
